 ...
 */
@Named(value = "modificarCargoEmpleado")
@ViewScoped
public class modificarCargo_Empleado implements Serializable {
    ...
    public modificarCargo_Empleado() {
    }
    ...
    public void modificar(CargoEmpleado cem) {
        cemp = cem;
        CargoEmpleadoPK pk = new CargoEmpleadoPK(cemp.getCargoEmpleadoPK().getIdEmpleado(),
                 cemp.getCargoEmpleadoPK().getIdOrganismo(), cemp.getCargoEmpleadoPK().getIdCargo(),
                cemp.getCargoEmpleadoPK().getIdCategoria());
        viejo = servicio.buscarPorPK(pk).get(0);
        isExecutedModificar = true;
    }

    public void confirmarCambio() {
        cemp.setCategoria(servicio4.buscarPorId(cemp.getCargoEmpleadoPK().getIdCategoria()).get(0));
        cemp.setEstado(true);
        servicio.borrar(viejo);
        servicio.agregar(cemp);
        exito = true;
        existe = false;
        isExcecuted = false;
        isExecutedModificar = false;
